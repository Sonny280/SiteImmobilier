...
export function AdminRealisations() {
  ...
  const toggleVisible = async (item) => {
    const res = await fetch(`${API}/realisations/${item.id}`, { method:"PUT", headers:{"Content-Type":"application/json", ...getAuthHeader()}, body:JSON.stringify({visible:item.visible?0:1}) });
      ...
      {modal && (
        <Modal open title={modal==="new"?"Nouvelle réalisation":"Modifier la réalisation"} onClose={()=>setModal(null)} wide>
          <div style={{display:"flex",flexDirection:"column",gap:"14px"}}>
            ...
            <Txta label="Description" rows={3} value={form.description} onChange={e=>sf("description",e.target.value)} placeholder="Détaillez le projet..."/>
            <div>
