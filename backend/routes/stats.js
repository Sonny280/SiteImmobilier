...
router.get("/", auth, async (_, res) => {
  try {
    ...
    // Biens
    const totalBiens    = (await prepare("SELECT COUNT(*) as c FROM biens WHERE statut!='archive'").get())?.c || 0;
    const biensDispo    = (await prepare("SELECT COUNT(*) as c FROM biens WHERE statut='disponible'").get())?.c || 0;
    const biensLoues    = (await prepare("SELECT COUNT(*) as c FROM biens WHERE statut='loue'").get())?.c || 0;
    const biensVente    = (await prepare("SELECT COUNT(*) as c FROM biens WHERE type='vente' AND statut!='archive'").get())?.c || 0;
    const tauxOccup     = totalBiens > 0 ? Math.round((biensLoues/totalBiens)*100) : 0;

    // Clients
    const totalClients  = (await prepare("SELECT COUNT(*) as c FROM clients").get())?.c || 0;
    const locataires    = (await prepare("SELECT COUNT(*) as c FROM clients WHERE type='locataire'").get())?.c || 0;
    ...
    const nbRetards     = (await prepare("SELECT COUNT(*) as c FROM loyers WHERE statut IN ('impaye','en_attente') AND echeance < ?").get(today))?.c || 0;
    ...
    const contratsExpir = (await prepare("SELECT COUNT(*) as c FROM contrats WHERE statut='actif' AND dateFin BETWEEN ? AND ?").get(today,in60))?.c || 0;

    // Demandes
    const demandesNouv  = (await prepare("SELECT COUNT(*) as c FROM demandes WHERE statut='nouveau'").get())?.c || 0;

    // Ventes
    const ventesEncours = (await prepare("SELECT COUNT(*) as c FROM ventes WHERE statut NOT IN ('finalisee','annulee')").get())?.c || 0;
    const ventesFin     = (await prepare("SELECT COUNT(*) as c FROM ventes WHERE statut='finalisee'").get())?.c || 0;
