...
router.post("/", contactLimiter, async (req, res) => {
  ...
  const r = prepare("INSERT INTO demandes (nom,email,tel,interet,budget,message,bienId,statut,source) VALUES (?,?,?,?,?,?,?,'nouveau','formulaire')")
    .run(nom, email||null, tel, interet||"location", budget||null, message||null, bienId||null);
  const demande = prepare("SELECT * FROM demandes WHERE id=?").get(r.lastInsertRowid);
  const bien    = bienId ? prepare("SELECT * FROM biens WHERE id=?").get(+bienId) : null;
...
router.put("/:id/statut", auth, requireModule("demandes"), (req, res) => {
  const { statut } = req.body;
  prepare("UPDATE demandes SET statut=? WHERE id=?").run(statut, +req.params.id);
  res.json(prepare("SELECT * FROM demandes WHERE id=?").get(+req.params.id));
});

// DELETE /api/demandes/:id
router.delete("/:id", auth, requireModule("demandes"), (req, res) => {
  prepare("DELETE FROM demandes WHERE id=?").run(+req.params.id);
