// database_sqlite.js — SQLite local (développement uniquement)
// Utilisé par database.js quand DATABASE_URL n'est pas défini
...
async function initDb() {
  ...
  _schema(_db); _migrate(_db); _db._save();
  return _db;
}
...
function _migrate(db) {
  ...
  const wanted = {
    type: "TEXT DEFAULT 'bail'", indexation: "TEXT",
    dateSignature: "TEXT", prixVente: "REAL",
    notaire: "TEXT", titreVerifie: "INTEGER DEFAULT 0",
  };
  for (const [col, def] of Object.entries(wanted)) {
    if (!existing.includes(col)) {
      try { db.run(`ALTER TABLE contrats ADD COLUMN ${col} ${def}`); }
      catch (e) { console.error(`Migration ${col}:`, e.message); }
    }
  }
}
...
function _schema(db) {
  db.run(`
    CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, nom TEXT NOT NULL, email TEXT UNIQUE NOT NULL, password TEXT NOT NULL, role TEXT DEFAULT 'admin', createdAt TEXT DEFAULT (datetime('now')), updatedAt TEXT DEFAULT (datetime('now')));
    ...
    CREATE TABLE IF NOT EXISTS temoignages (id INTEGER PRIMARY KEY AUTOINCREMENT, nom TEXT NOT NULL, profession TEXT, note INTEGER DEFAULT 5, texte TEXT NOT NULL, statut TEXT DEFAULT 'en_attente', createdAt TEXT DEFAULT (datetime('now')));
    CREATE TABLE IF NOT EXISTS realisations (id INTEGER PRIMARY KEY AUTOINCREMENT, titre TEXT NOT NULL, type TEXT DEFAULT 'Gestion locative', description TEXT, annee TEXT, commune TEXT, ville TEXT DEFAULT 'Abidjan', image TEXT, ordre INTEGER DEFAULT 0, visible INTEGER DEFAULT 1, createdAt TEXT DEFAULT (datetime('now')));
    CREATE TABLE IF NOT EXISTS documents (id INTEGER PRIMARY KEY AUTOINCREMENT, nom TEXT NOT NULL, type TEXT DEFAULT 'autre', entite TEXT NOT NULL, entiteId INTEGER NOT NULL, fichier TEXT NOT NULL, taille INTEGER, mimeType TEXT, notes TEXT, createdAt TEXT DEFAULT (datetime('now')));
    CREATE TABLE IF NOT EXISTS articles (id INTEGER PRIMARY KEY AUTOINCREMENT, titre TEXT NOT NULL, slug TEXT UNIQUE, categorie TEXT DEFAULT 'Actualités', resume TEXT, contenu TEXT, auteur TEXT DEFAULT 'ImmobilierCI', statut TEXT DEFAULT 'brouillon', image TEXT, tags TEXT, vues INTEGER DEFAULT 0, createdAt TEXT DEFAULT (datetime('now')), updatedAt TEXT DEFAULT (datetime('now')));
...
function prepare(sql) {
  return {
    ...
    get(...p) {
      if (!_db) throw new Error("DB not ready");
      const s = _db.prepare(sql); s.bind(p.map(v => v===undefined?null:v));
      const row = s.step() ? s.getAsObject() : null; s.free();
      return row;
    },
    all(...p) {
      if (!_db) throw new Error("DB not ready");
